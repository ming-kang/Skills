...
- **Any blocked** → surface each task's `## Blocked` entry first — the user may have already resolved it outside the cycle
- **Any in_progress, blocked, or ready** → list them and ask the user how to close:
  - Continue work
  - Mark selected tasks
  - Archive as-is (blocked tasks land in Gaps & Follow-Ups with their blocking reason)
