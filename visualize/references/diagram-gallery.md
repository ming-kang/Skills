...
| Swimlane | `assets/gallery/swimlane.svg` | Three horizontal role lanes as dashed containers; the story is the arrows **crossing** lane boundaries (handoffs), plus a terracotta out-of-stock return path. |
| Tree / org chart | `assets/gallery/tree.svg` | Three-level hierarchy from nested `fanout()` buses; sibling teams use tint-within-family (`opacity=0.55`) instead of extra colors. |
