...
- `blocked` → the same step failed twice or a constraint conflict needs the user; the task's `## Blocked` section records what was tried and what would unblock it
- `completed` → work is done and verified

Two pacing rules the task files also carry, so they survive hand-off to another agent:

- **Stop before you spiral.** Two failures on the same step means the approach is wrong or information is missing — mark `blocked` and surface it instead of burning the budget on variations.
- **One task at a time.** A finished task is a stopping point, not a springboard; report and let the user choose what runs next. This is what makes parallel dispatch safe — the user, not momentum, allocates tasks.
