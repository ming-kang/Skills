...
def write_temp_xml(xml_content):
    """Create a temporary XML file that can be reopened on Windows."""
...
class TestSafeParseXml:
    ...
    def test_valid_xml_file(self):
        ...
        file_path = write_temp_xml('<?xml version="1.0"?><root><child/></root>')
        try:
    ...
    def test_duplicate_attribute_file(self):
        ...
        file_path = write_temp_xml(xml_content)
        try:
            tree = safe_parse_xml(file_path)
            root = tree.getroot()
