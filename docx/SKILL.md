...
**Creating new documents**: Use C# SDK with `<skill-path>/scripts/docx build` → See `Example.cs` for patterns, `CJKExample.cs` for CJK content
...
**Windows support expects Git Bash.** Inside Bash, prefer relative paths or POSIX absolute paths like `/c/Users/name/report.docx`; drive-letter paths like `C:\Users\name\report.docx` are also accepted.
...
| Temp work root | `./.docx-tmp/` | Per-build temporary workspace copied from `./.docx/` |
...
**Runtime workspace naming under `./.docx-tmp/`**:

- Build: `docx-task-*`
- Python validate/fix: `docx-validate-all-*`, `docx-validate-docx-*`, `docx-fix-element-order-*`
- Python editing: `docx-edit-*`
- Successful runs clean up by default; failed runs are preserved for debugging

**Environment Overrides**:

- `DOCX_TASK_ROOT` — override the task root for CLI and Python runtime helpers
- `DOCX_SOURCE_DIR` — override the persistent source workspace
- `DOCX_WORK_ROOT` — override the shared work root for build, validate, fix, and edit workspaces
- `DOCX_OUTPUT_DIR` — override the default output directory for generated docs
- `DOCX_KEEP_WORKSPACE=1` — keep successful workspaces for debugging
- `DOCX_ASSET_OUTPUT_DIR` — default output directory for helper asset scripts

**Script Commands** (`<skill-path>/scripts/docx <cmd>`):
...
- Detects `dotnet`, the local OpenXML validator DLL, and `python`/`python3`
- Reports `lxml` as required for Python editing workflows
- Reports `pandoc`, `playwright`, and `matplotlib` as optional extras with install guidance
- Creates `./.docx/` once, then uses per-run visible workspaces under `./.docx-tmp/`
- `docx env` reports the full build/validate/edit workspace contract and supported editing overrides
- Cleans successful workspaces automatically and preserves failed ones for debugging

## Build Process

**Must use `<skill-path>/scripts/docx build`**, do not execute `dotnet build && dotnet run` separately (skips validation and local workspace handling).
...
with DocxContext("./report.docx", "./report-edited.docx") as ctx:
...
- default edit workspaces live under `./.docx-tmp/docx-edit-*`
- successful edits clean up by default; failed edits are preserved
- optional keyword-only overrides: `task_root=...`, `work_root=...`, `keep_workspace=...`, `keep_failed_workspace=...`
- if no keyword overrides are passed, `DOCX_TASK_ROOT`, `DOCX_WORK_ROOT`, and `DOCX_KEEP_WORKSPACE` still apply
